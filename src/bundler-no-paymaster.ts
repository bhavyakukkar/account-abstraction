...
async function main() {
  ...
  const { EP_ADDRESS, AF_ADDRESS } = await deployContracts();

  await question("Contracts deployed, press <Enter> after starting Bundler with the deployed EntryPoint");

  const bundler =
    new hre.ethers.providers.JsonRpcProvider("http://localhost:3000/rpc");
  ...
  const af =
    await hre.ethers.getContractAt("SimpleAccountFactory", AF_ADDRESS);
  ...
  const callData = keccak256(Buffer.from('entryPoint()')).slice(0, 10);
  // const callData = acc_factory.interface.encodeFunctionData("execute", [
