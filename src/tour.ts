...
async function main() {
  ...
  const me = Array.isArray(accounts) ?
    ...
    Wallet.fromMnemonic((accounts as HardhatNetworkHDAccountsConfig).mnemonic, (accounts as HardhatNetworkHDAccountsConfig).path + '/0');
  ...
  // Construct of User Operation
  {
    ...
    signedUserOp = signUserOp(userOp as UserOperation, me, ep.address, Number.parseInt(await getChainId()));
  }
  console.log("User-Operation:", signedUserOp);

  // try sending user-op without staking any deposit
  try {
    await ep.handleOps([signedUserOp], beneficiary.address, {
      gasLimit: 200000,
    });
  }
  catch (e) {
