...
import { hashMessage, keccak256, recoverPublicKey, verifyMessage } from "ethers/lib/utils";
...
import { ECDSA__factory } from "../typechain";
...
async function main() {
  ...
  // Delegate to the entrypoint the duty of initializing our Account contract by passing the
  // calldata to deploy it (`AccountFactory.createAccount`)
  ...
  // The call executed by the Entrypoint on the Account contract after the UserOp is verified &
  // validated
  const callData =
  ...
  const userOp: UserOperation = {
    ...
    paymasterPostOpGasLimit: "0x" + (500000000).toString(16),
    paymasterVerificationGasLimit: "0x" + (50000000).toString(16),
    paymasterData: "0x",

    // needs to be at least 100000, hardcoded here:
    // https://github.com/eth-infinitism/bundler/blob/master/packages/sdk/src/BaseAccountAPI.ts#L147
    // verificationGasLimit: "0x1343af9",
    verificationGasLimit: "0x" + (20200185 * 1).toString(16),
    // preVerificationGas: "0xd118",
    preVerificationGas: "0x" + (53528 * 1).toString(16),

    // call gas fields
    // callGasLimit: "0x" + (65 * 850000000).toString(16), //works when pm deposit = 100 ETH
    callGasLimit: "0x" + (65 * 1000000000).toString(16), //works when pm deposit = 120 ETH
    maxFeePerGas: "0x" + (2022527136 * 1).toString(16),
    maxPriorityFeePerGas: "0x" + (1500000000 * 1).toString(16),

    signature: NULL,
  };
  console.log({ userOp });

  // Sign the incomplete UserOp so that it can still be validated when passed our account's
  // validateUserOp during off-chain simulation via `EntryPoint.simulateHandleOp` called by
  // bundler's `eth_estimateUserOperationGas` rpc method
  ...
  console.log({ packedUserOp });
  // 0x0165878a594ca255338adfa4d48449f69242eb8f00000000000000000000000002faf0800000000000000000000000001dcd6500

  // sign the paymasterData as the paymaster
  const signedPaymasterData = await my_eoa.signMessage(hre.ethers.utils.arrayify(paymasterData));
  console.log({ signedPaymasterData });
  userOp.paymasterData = "0x" +
    "0".repeat(32 * 2) + //validUntil
    "0".repeat(32 * 2) + //validAfter
    signedPaymasterData.slice(2);

  // sign the userop again since our paymasterData field has changed
